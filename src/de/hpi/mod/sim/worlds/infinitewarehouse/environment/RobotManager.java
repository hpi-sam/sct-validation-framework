...
public class RobotManager implements IRobotController, ILocation, IScanner, IRobotDispatch { 

    private List<Robot> robots = new CopyOnWriteArrayList<>();
    private GridManager gridData;
    ...
    public RobotManager(GridManager grid, IRobotStationDispatcher stations) {
        this.gridData = grid;
        this.stations = stations;
    }

    /**
     * Creates a new Robot and adds it to a charger
     * 
    ...
    public Robot addRobot() {
        ...
        Robot robot = new Robot(
                ...
                getChargerPositionAtStation(stationID, chargerID),
                Orientation.EAST);
        robots.add(robot);
        return robot;
    }
    ...
    public Robot addRobotAtPosition(Position position, Robot.RobotState state, Orientation facing, List<Position> targets,
        ...
        Robot robot = new Robot(robotID, 0, gridData, stations, this, this, position, state, facing, targets, delay,
                initialDelay, fuzzyEnd, unloadingTest, hasReservedCharger, hardArrivedConstraint);
        robots.add(robot);
        return robot;
    }
